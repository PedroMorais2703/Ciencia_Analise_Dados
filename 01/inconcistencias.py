...
df['data_atualizada'] = df['data'].where(df['data'] <= data_atual, pd.to_datetime('01-01-1900'))
...
df['estado_sigla'] = df['estado_sigla'].str.upper().apply(lambda x: x if x in estados_br else 'Estado Desconhecido' )

print('Dados trancados: \n', df.head())
...
df_salvar = df[['nome', 'cpf', 'idade', 'endereco', 'estado', 'data', 'estado']]
