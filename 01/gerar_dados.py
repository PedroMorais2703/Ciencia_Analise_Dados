import pandas as pd
import random # este random n esta funcionando
from faker import Faker

faker = Faker('pt_br')

dados_pessoas = []

for _ in range(10):
    nome = faker.name()
    cpf = faker.cpf()
    idade = faker.randint(18, 60)
    data = faker.date_of_birth(minimum_age=idade, maximum_age=idade).strftime("%d/%m/%y")
    endereco = faker.address()
    estado = faker.state()
    pais = 'Brasil'


    pessoa = {
        'nome': nome,
        'cpf': cpf,
        'idade': idade,
        'data': data,
        'endereco': endereco,
        'estado': estado,
        'pais': pais,
    }

    dados_pessoas.append(pessoa)

df_pessoas = pd.DataFrame(dados_pessoas)
print(df_pessoas)

# opções de pesonalização de exibição(para não cortar linha)
pd.set_option('display.max_columns', None)
pd.set_option('display.max_rows', None)
pd.set_option('display.max_coldwidth',None)
pd.set_option('display.max_width',None)

print(df_pessoas.to_string())

df_pessoas.to_csv('clientes.csv') #salvando em arquivo csv